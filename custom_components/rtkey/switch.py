...
async def async_setup_entry(hass, config_entry, async_add_entities):
    ...
    for intercom_info in intercoms_info["data"]["devices"]:
        ...
        entities.append(RTKeySwitchEntity(hass, config_entry, cameras_api, intercom_info, camera_info))
...
class RTKeySwitchEntity(SwitchEntity):
    def __init__(
        ...
        if (camera_info):
            ...
            self.device_name = self.cameras_api.build_device_name(intercom_info["name_by_company"])
    ...
    def device_info(self) -> DeviceInfo:
        return {
            "identifiers": {(DOMAIN, f"{self.config_entry_id}_{self.camera_id if self.camera_id else self.intercom_id}")},
